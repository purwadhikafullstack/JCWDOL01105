...
const OrderComponent = () => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        const response = await api.get(
          `/orders?sortBy=${sortBy}&page=${currentPage}&booking_status=${status}`,
        );
  ...
  const updateFetching = async () => {
    try {
      const response = await api.get(
        `/orders?sortBy=${sortBy}&page=${currentPage}`,
      );
  ...
  return (
    <div>
      ...
      <div className="flex flex-col max-w-7xl mx-auto gap-8 w-full mt-6">
        ...
        {orders.length === 0 ? (
          <div className="flex flex-col w-full max-w-sm px-4 mx-auto mb-28">
